...
class SaliencyHead(nn.Module):
    ...
    def forward(self, inputs, data, output, mode):
        ...
        for i, x in enumerate(inputs):
            ...
            # Create new column matching the current batch size and sequence length
            new_column = torch.full((x.shape[0], x.shape[1], 1), VALUE_TO_ADD)

            # Concatenate
            x = torch.cat((x, new_column), dim=-1)
